    ...
    try {
      ...
      try {
        for (var _iterator2 = diagrams[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
          ...
          created.push(y.id);
          var graphId = renderJSON(y, callback);
...
function getRoughEdgeList(graph, objInfo, grid) {
    ...
    graph.roughEdges.forEach(function (roughEdge) {
      ...
      if (roughEdge.source && roughEdge.target && roughEdge.target.to) {
        ...
        if (sourceId in objInfo && targetId in objInfo) {
          ...
          roughEdgeList.push({
            edge: [Math.ceil((sourceInfo.x + sourceInfo.width) / granularity), Math.ceil(sourceInfo.y / granularity), Math.floor(targetInfo.x / granularity), Math.floor(targetInfo.y / granularity)],
  ...
  graph.children.forEach(function (child) {
    roughEdgeList.push.apply(roughEdgeList, _toConsumableArray(getRoughEdgeList(child, objInfo, grid)));
  });
...
function getPFGrid(gridSize, obstacleInfo, objInfo) {
  ...
  var grid = new PF.Grid(Math.ceil(gridSize.x / granularity), Math.ceil(gridSize.y / granularity));
  ...
  obstacleInfo.forEach(function (obstacle) {
    if (obstacle.type == "node") {
      ...
      var holeYs = (objInfo[obstacle.id].labels || []).map(function (label) {
        return Math.floor(label.y / granularity);
      });
...
function layoutRoughEdges(graphs) {
  ...
  var grid = getPFGrid(gridSize, obstacles, objInfo);

  var roughEdgeList = [];
  graphs.forEach(function (graph) {
    roughEdgeList.push.apply(roughEdgeList, _toConsumableArray(getRoughEdgeList(graph, objInfo, grid)));
  });
